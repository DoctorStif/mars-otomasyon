...
async def is_emri_isle(page, is_no, miktar, recete_no, makine_no):
    ...
    uretim_btn = await page.query_selector(
        "button:has-text('Üretim'), .btn-success:has-text('Üretim'), .btn:has-text('Üretimde')"
    )
    if not uretim_btn:
        log(f"  ⚠ Üretim butonu bulunamadı: {is_no}", "uyari")
        return False

    await uretim_btn.click()
    await page.wait_for_timeout(1500)

    modal = await page.query_selector(".modal, [role='dialog']")
    if not modal:
        log(f"  ⚠ Modal açılmadı: {is_no}", "uyari")
        return False

    miktar_input = await page.query_selector(
        "input[placeholder*='Onaylı'], input[placeholder*='Miktar'], "
        ".modal input[type='number']:first-of-type, .modal input[type='text']:first-of-type"
    )
    if miktar_input:
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .log-basari{ color: var(--success); }
  .log-uyari { color: var(--warn); }
  .log-hata  { color: var(--danger); }
...
<div class="card">
  ...
  <div class="log-box" id="logBox"><span class="log-info muted">Henüz işlem yapılmadı...</span></div>
</div>
...
async function baslat() {
  ...
  const res = await fetch('/baslat', {
    ...
    body: JSON.stringify({ kullanici, sifre, is_emirleri: emirler, miktar, tekrar_dk: tekrar, recete, makine })
  });
